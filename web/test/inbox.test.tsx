...
test('a visitor sees the API queue and can open a request', async () => {
  globalThis.fetch = async () => Response.json([
    { id: 1, customer_name: 'Maya Chen', subject: 'Cannot invite my team', status: 'open', priority: 'high', created_at: '2026-09-20T10:00:00.000Z' }
  ]);
...
test('a visitor reads a request and its history in the focused view', async () => {
  const summary = { id: 1, customer_name: 'Maya Chen', subject: 'Cannot invite my team', status: 'open', priority: 'high', created_at: '2026-09-20T10:00:00.000Z' };
  globalThis.fetch = async (input) => Response.json(String(input).endsWith('/tickets/1')
    ? { ...summary, question: 'Invites do not arrive.', history: [{ id: 1, description: 'Request received', created_at: summary.created_at }] }
    : [summary]);
  const html = renderToStaticMarkup(await TicketPage({ params: Promise.resolve({ id: '1' }) }));
...
test('the seeded inbox can be browsed from the UI through the API', async () => {
  ...
  database.public.none(readFileSync(new URL('../../db/seed.sql', import.meta.url), 'utf8'));
  ...
  try {
    ...
    assert.ok(ticketUrl);
    const detail = renderToStaticMarkup(await TicketPage({ params: Promise.resolve({ id: ticketUrl.split('/').at(-1)! }) }));
